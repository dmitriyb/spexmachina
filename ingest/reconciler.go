...
// TODO(bead:spexmachina-y0wc.32): Reconciler wrote spec-node-to-task
// linkage into mapping.Record/.bead-map.json, both retired by
// spexmachina-y0wc.19's migration of MappingStore onto the journal
// (spec/.history.jsonl) — ingest now appends journal events/receipts
// instead of reconciling a record store. Re-derive the per-op transition
// table against journal appends per spec/ingest/arch_reconciler.md and
// re-enable. SpecGraph/NodeMetadata/ReconcileSummary above carry no
// mapping.Record dependency and are kept live for reuse.
//
