...
type Change struct {
	...
	NodeType string     // type name the resolved profile declares, or "meta" for envelope leaves; vocabulary is open per profile (default profile: "component", "data_flow", "test_section", "api", "requirement"). A removed leaf carries whatever the snapshot recorded, which may fall outside the current profile's declarations.
