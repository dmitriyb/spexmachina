...
func TestReadBeadsBytes_WrappedShape(t *testing.T) {
	...
	want := []BeadSpec{{ID: "sm-1", Status: "open"}}
...
func TestReadBeadsBytes_BareArrayShape(t *testing.T) {
	...
	want := []BeadSpec{{ID: "sm-2", Status: "closed"}}
	if !reflect.DeepEqual(got, want) {
		t.Errorf("got %#v, want %#v", got, want)
	}
}
...
// TestReadBeadsBytes_NoLabelFiltering verifies every bead is carried
// through regardless of its labels — BeadReader parses no labels and drops
// no beads on their account. This is a regression guard for the retired
// label-parsing behavior (arch_bead_reader.md, "No Label Parsing").
func TestReadBeadsBytes_NoLabelFiltering(t *testing.T) {
	data := []byte(`[
...
// through untouched for downstream cleanup-bead gating.
...
func TestS1_BeadReaderCarriesIDAndStatus_ParsesNoLabels(t *testing.T) {
	...
	// BeadSpec's only fields are ID and Status: verify reflectively that no
	// third field sneaks in carrying anything derived from a label.
	typ := reflect.TypeOf(BeadSpec{})
	if typ.NumField() != 2 {
		t.Fatalf("BeadSpec has %d fields, want 2 (ID, Status) — no label-derived field", typ.NumField())
	}
}
