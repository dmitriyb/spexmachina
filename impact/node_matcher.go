...
// TODO(bead:spexmachina-y0wc.23): NodeMatcher correlated classified changes
// against mapping.Record, the CRUD-store type spexmachina-y0wc.19 removed
// when it migrated MappingStore onto the journal (spec/.history.jsonl).
// Under the new architecture NodeMatcher should correlate against live
// tracker beads (BeadReader's output) directly by identity-hash label, not
// against a mapping-store record slice. Re-derive Match/Unmatched/Orphaned
// per spec/impact/arch_node_matcher.md and re-enable this file.
//
