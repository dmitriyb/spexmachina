...
6. **Update everything the change touches — graph edges *and* prose.**
   - *Edges*: update all graph edges affected by the change — e.g., if a component is removed, remove its ID from any `uses` or `describes` arrays.
   - *Prose footprint*: a removed or renamed component, command, flag, or identifier can be named as prose in content leaves of **any** module — pipeline diagrams, narrative, examples — not only the module the proposal names. `git grep` the old name across the whole `spec/` tree. Rewrite every leaf that still presents the removed or renamed thing as current; keep deliberate negative or historical mentions (e.g. "there is no `spex hash` command"). Scope the edits to what the grep finds, not to the modules the proposal happened to enumerate.
