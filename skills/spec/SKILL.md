...
**A leaf that relies on another module's contract declares the dependency.** When a leaf cites another module's command, exit code, file, pre-flight or ordering, it links the node it relies on, and `project.json` carries `requires_module` from this module to that one (`spex edge add <module-id> requires_module <module-id>`). A paraphrase with no link and no edge is invisible to every mechanical check and to the review's scope walk; `/spec-review`'s undeclared-dependency lens is the only thing that catches it, and only when that leaf is in scope.
