...
digraph proposal_lifecycle {
    ...
    "changeset.json"          -> "tracker tasks"           [label="scripts/apply-br.sh"];
    ...
    "tracker tasks"           -> "97f73ced5a02"            [label="br list --json | spex log"];
}
```
...
the diff, changeset and receipts the middle of the pipeline writes, and the tasks themselves, which live in
whatever tracker the user runs. Two of those artifacts carry more than their name says:
`changeset.json` leads with the proposal-epic create op and follows it with the ordered task ops,
and `receipts.json` carries a task id for every op the adapter actually executed. Two steps leave
...
snapshot: the tasks reach it as JSON on stdin, read and parsed by the command that fronts it, which
...
first op creates the proposal epic task, and every other op in the batch is
...
7. The receipts produce tasks and their journal events.
...
- HistoryViewer (`spex log`) never reads anything Registrar wrote beyond that
  file: it groups tasks by the `spec_proposal:<stem>` label already on each
  task, then re-opens `spec/proposals/<stem>.md` at render time to read the
...
- Carried by the proposal-epic create op: `spec_node_kind = "proposal_epic"`,
  ...
  `registered` event, read from the run's registration (the parsed event, not the fold), and ingest pairs the epic's `task_created`
  to that event through `for` — no spec-graph lookup, no side channel.
- Used by HistoryViewer (`spex log`) to group tasks under the proposal
...
- ProposalLog (`--json`):
  - proposals: list, one entry per `spec_proposal:` stem, in ascending stem order
    ...
    - tasks: list, in the order the input supplied them
      - id: string — the tracker task id
      - status: string — the task's status as the input reported it
      - action: string — `created` or `closed`, derived from that status
      - summary: string — the task's title

Grouping reads the `spec_proposal:` label and nothing else. No epic task id is
