...
- `spec/apply/` — entire module (15 files). The apply module (project requirement 5 in `spec/project.json`) is superseded by emit + ingest.
...
- `spec/impact/module.json` — rewrite BeadReader (component 3) description to reflect pure-function input parsing; add a requirement for `--beads` input ingestion; update ImpactCommand and ActionClassifier descriptions to note that bead status is sourced from enriched mapping records.
...
- impact (module 4): one `feature` bead for the BeadReader rewrite plus the `--beads` flag wiring in the ImpactCommand component.
- proposal (module 6): one `feature` bead for the HistoryViewer stdin rework.
- One `feature` bead for `scripts/apply-br.sh` and its integration test (the bead can hang off a delivery or cli module; exact home determined at `/spec` time).
- **Code cleanup task beads** generated by the ActionClassifier for each removed apply component whose bead is closed by the time this proposal runs: BeadCreator (component 1), BeadCloser (component 2), ProposalTagger (component 4), SnapshotSaver (component 5), ApplyCommand (component 6). Five cleanup beads total, each carrying the work of deleting the corresponding Go source under `apply/` and the entrypoint in `cmd/spex/apply.go`.
...
**Closed beads**: All five apply-module beads are obsoleted and closed via `spex:obsolete` + `commit:<HEAD>` labels. Their mapping records are deleted per the existing behavior at `apply/bead_closer.go:57` (`ChangeType == "removed"`). The cleanup task beads generated alongside them carry the source-deletion work.
