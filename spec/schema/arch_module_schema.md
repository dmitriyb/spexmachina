...
The effective document is composed, not fixed: the shipped schema is the frame, and the resolved profile supplies one array property per module-scoped node type it declares, keyed by that type's plural key, each validated with the same envelope constraints. The default profile declares today's five arrays, and the composed document it yields is identical in content to the shipped one — the same JSON value, which a golden test asserts by structural comparison — so the on-disk shape of every existing module.json is unchanged, while a project declaring an `endpoint` type gets an `endpoints` array validated exactly as `components` is.

The profile's legal edges reach the composed document too. For a declared type the frame does not already define, the synthesized entry definition carries — besides the envelope of identity-hash `id`, non-empty `name`, optional `description` and, when the type requires content, a required `content` path — one property per profile-declared edge kind whose source names that type, validated as an array of identity-hash values. A field that is neither envelope nor declared edge is still rejected by the entry-level `additionalProperties: false`, so declaring an edge is the only way to open a reference field on a declared type — the composed schema is what makes the edge half of [[5392cca550c4|the taxonomy declaration]] carriable at all. An edge declared through the profile always composes as an array; the one scalar reference field, `preq_id`, is a shape of the frame's own requirement definition, and the built-in types keep their frame definitions — reference fields included — unchanged. A new edge kind sourced at a built-in type therefore has nowhere to compose, and profile validation rejects the declaration outright — one early error naming it — rather than accept an edge no document could carry.
