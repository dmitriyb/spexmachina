...
- Compose the effective [[79946d618829|ProjectSchema]] and [[78883b84c32d|ModuleSchema]] documents from the resolved profile, once per run and before any check — [[ef55248cb3ca|the schemas are generated from the profile]], with the embedded frame supplying envelope fields, the identity-hash pattern and `additionalProperties: false`, and the profile supplying the array properties per declared node type. Under the default profile the composition reproduces the shipped static documents' content — the same JSON value, which the golden test asserts by structural comparison. Byte equality is deliberately not the contract: the shipped files are hand-formatted with indentation and a deliberate key order, while the composer emits compact key-sorted JSON, and nothing consumes the documents as bytes — every caller compiles them.
