...
Which node types produce beads is the resolved profile's plan-relevant declaration; the bead-type column is the adapter's own kind-to-bead-type mapping, never the profile's or the changeset's. A profile-declared plan-relevant type's creates carry that type's name in `spec_node_kind` verbatim — no new op shape — and the adapter's mapping resolves it, its default arm filing a kind it does not know as `feature`. Under the default profile the kinds emitted are exactly the rows above.
