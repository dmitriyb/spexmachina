...
- Take the classified actions (create, close, retarget) and classify the creates by spec node type tier (proposal epic / feature+data_flow task / multi-component test task / cleanup).
- **Detect cleanup actions** by the `"Code cleanup:"` prefix on the action's reason. Cleanup actions get a distinct op shape — see "Cleanup op shape" below — and their deps are composed here from the batch itself, not resolved from `DepSpecNodeIDs`: a cleanup action carries none.
...
- Emit no lineage. A create for a modified node whose earlier task finished is the same op a fresh node gets: no dep naming the predecessor, no field carrying its id. The journal's event chain is where one generation meets the next.
- Write the absorbed entries PlanCommand composed into the top-level `absorbed` array — see "Absorbed array" below.
- Write the final v4 changeset with canonical field order and stable key ordering inside every nested object.
...
Three op kinds flow through the builder, and three is the whole vocabulary:

| Op type | Emitted for | Adapter action |
|---------|-------------|----------------|
| `create` | a new node, a modified node whose earlier task is finished, a removed node's cleanup, the proposal epic | create a task in the tracker |
| `close`  | a removed node's open task, or a folded-back test_section's open task | close the task |
| `retarget` | a modified node whose open task moves to the new state | update the task: add the event label, add missing deps |

Those three names are [[77f5182ac5f7|the whole vocabulary a changeset may use]], and the bar that requirement sets is that no tracker-specific flag or subcommand name appears in `changeset.json`; the adapter column above is a mapping the adapter owns, not something the changeset states. That is what lets one changeset drive an adapter for a different tracker unchanged. The `label` and `tag` kinds the v3 vocabulary reserved were never emitted and left with v4: a vocabulary is closed only when it lists what is produced, and an adapter that had to answer kinds nobody minted was carrying dead weight. Retiring the modify pair's lineage dep and renaming the task ref shape are what forced [[7f275787df34|the version bump to 4]]: a v3 consumer meeting a `ref:task` would resolve nothing and must refuse the document, never execute half of it.

### `spec_node_kind` vocabulary

Create ops carry `spec_node_kind` matching the underlying spec node category:

| Kind | When emitted | Tracker type via adapter mapping |
|------|--------------|-------------------------------|
| `proposal_epic` | First op of a fresh proposal — represents the proposal as an epic | `epic` |
| `component`     | New or successor component arch leaf | `feature` |
| `data_flow`     | New or successor data_flow leaf | `task` |
| `test_section`  | New or successor test_section leaf with `describes >= 2` | `task` |
| `cleanup`       | Code-cleanup task for a removed spec node — see "Cleanup op shape" below | `task` |
...
| `target` | `{ref:task, task_id:"<the open task>"}` — the task whose target moves |
...
No `parent`, no `priority`, no `title`, no `body`: the task already exists with all four, and a retarget moves only its target state and deps. No close accompanies it and no lineage dep is minted — the accumulation is readable from the journal's `task_retargeted` receipts instead.
...
| `deps`            | every non-cleanup, non-epic create op in the batch as `ref:op`, and each retarget op's target task as `ref:task`, in op order — the batch's other work, so the cleanup is ready only when the last of it closes. The finished task the cleanup follows is not named; the journal's `removed` event is the tie |
...
The deps are the batch's other work rather than nothing because a removed node's code is not necessarily dead. The diff reports a rename as one `removed` and one `added` with nothing tying them together, so the node a cleanup answers may be the old half of a rename whose code a live node still calls until the new half and the caller's own change land — and those land through creates and retargets in this same batch. Nothing the builder reads says which ops those are: the spec graph is the post-change one and the snapshot carries no edges. So the cleanup waits for every non-cleanup, non-epic create and every retarget of the batch, and the only failure that rule admits is over-waiting, which the changeset shows. The epic is excluded because it is the cleanup's parent, a grouping rather than work, and an edge on it would never clear; other cleanups are excluded because they delete independently. Ops that landed in an earlier, partial run are outside the batch and are not named — a re-run's cleanup waits only for what the re-run carries, and a cleanup that is the whole batch carries no deps at all.
...
A ref carries its discriminator and one id, and nothing else: no edge-type key, because the lineage edge was the only typed dep and it is gone. The adapter applies every dep as its default blocking edge.
...
Every create op carries a title the adapter files as the task's title without editing it:
...
The body is the spec context a reader of the task needs and nothing beyond it: the repo-relative path of the node's own content leaf, and the path of the `module.json` that declares it. A node with no content leaf on disk — the proposal epic — gets an empty body, and so does every cleanup create. The body is markdown; the adapter passes it through to the tracker's description field unchanged.
...
Together those five rules are [[7f275787df34|what makes two runs over identical inputs byte-identical]], which is what lets `changeset.json` be reviewed in a git diff rather than merely re-parsed. Every label in the document is derived from the op that carries it or read from the fold; none is predicted from where another block of ops will be numbered, which is what the earlier cross-block cleanup label needed and what made its numbering fragile.
...
bundled with that component's own implementation task.
