...
The vocabulary is closed, and `api` is deliberately not in it. An api is a declared external surface, not a unit of work, and the classifier produces no action for one, so no action carrying an api ever reaches `Build()`.
