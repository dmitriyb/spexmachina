...
One call, taking either a stream to read from or the bytes already in hand. Each entry it returns carries two things: the tracker's own bead id (`spexmachina-abc` and the like) and the bead's live status exactly as the input reported it. A failure is returned to the caller rather than printed, and every message begins `plan: read beads:`.
