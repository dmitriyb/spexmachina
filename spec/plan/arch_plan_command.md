...
[[ca0f477fdded|`spex plan`]] is entered here. One invocation covers what used to be a two-command seam: it reads a merkle diff, refuses it if `spex diff` flagged it as incomplete, enriches the journal fold's pairings with live status from the required `--tasks` artifact, maps changed spec nodes to actions, and composes `changeset.json` — there is no intermediate document and no second command between the diff and the changeset.

## Usage

```
spex plan --proposal <ref> --git-head <sha> --tasks <file> [--diff <file>] [--absorb <file>] [--out <file>]
```
...
| `--tasks`    | yes | The task-state artifact — the version-1 document the adapter's export half derived from the tracker, listing in-flight tasks only — parsed and validated by TaskReader. |
...
The root's persistent `--spec-dir` is read too: it is what the spec graph is loaded from. The task journal lives inside `.spex/` at the location the lifecycle pre-flight answers, and the absorb file, being run scratch rather than authored spec or durable state, conventionally sits under the state directory's runs area (`.spex/runs/absorb.json`); `--absorb` takes whatever path the caller committed it at. The task-state artifact is run scratch of the same kind — regenerated by the export half before every run — and conventionally sits beside it.
...
3. Parse and fold the task journal at its resolved location — an absent file still folds empty at this layer, though the lifecycle pre-flight refuses an uninitialised or broken project before the fold is reached. From the same parsed events, resolve the run's registration: the `registered` event whose proposal is `--proposal`, or nothing if the journal holds none. One parse answers both — the fold lists task-bearing pairings, the registration is a lifecycle fact that has no pairing until the epic's task lands.
   The fold reaches the run as two projections, and **a removed node's entry — its tombstone — belongs to neither**. It is withheld from the pairings handed to [[972faea162a6|NodeMatcher]], because a node removed in an earlier epic and later re-added under the same name carries the same identity hash: matched against its own tombstone it would be read as a change to a finished task's node rather than as new work, and while both paths now yield a plain create, the tombstone's entry names either the cleanup task or the cancelled task — neither is work on the re-added node — and nothing about a re-add should be decided by it. It is withheld again from the lookup [[e9a3b1b85953|Resolver]] reads for epic and parent resolution, because a dead epic's tombstone would otherwise answer as that epic and parent every op in the run at a finished task. The epic-pairing entries the fold also carries are the mirror case — kept in the Resolver lookup, which keys on the proposal slug, and withheld from matching, which keys on a spec node hash they do not have. Neither projection is the fold verbatim; each drops what it cannot answer for.
4. Load the spec graph rooted at the spec directory's `project.json`.
5. Hand `--tasks`'s bytes to [[80afb22dab75|TaskReader]] and [[ac02e911999d|join each listed task's live status onto its journal pairing by task id]]. A pairing whose task the artifact does not list joins no status, and that absence is what the classifier reads as "finished": the cleanup gate fires for such a removed node, and a changed node with such a task gets a plain create. Nothing defaults, because there is no optional path — the artifact is required and an empty one is an explicit nothing-in-flight.
6. If `--absorb <file>` is given, parse it and [[049e8ae9cc51|validate every entry against the diff]]: each marked node must appear as `modified`, and a mark on an `added` or `removed` node, or on a node absent from the diff, is exit 2 naming the node. Each valid mark then removes its node's change from the stream handed to matching, and the command composes that node's absorbed entry — identity hash, before/after hashes off the diff entry, the authored reason. Absorption happens here, before any pairing or status is consulted, so a cosmetic edit never retargets, never refuses, and never opens a successor, whatever the state of the task tracking it; the builder only writes the finished entries.
...
- The adapter's export half runs before this command and its apply half after it, so one cycle reads the tracker exactly once, through the one component that owns its format — for the reference adapter, `export-br.sh`, then `spex plan`, then `apply-br.sh`, then `spex ingest`.

## Non-Responsibilities

- Does not run the adapter, in either half.
...
validation, the required-artifact refusal) live in the `Plan command tests`
test_section and ship with this component's implementation task. The
four-component composition that produces the changeset itself
(ChangesetBuilder + Resolver + TopologicalSorter + IdempotencyLabeler) is
covered by the consolidated `test_changeset_builder` test_section —
PlanCommand wires that composition but the cross-component integration
assertions live with the builder tests, not here — and the
classifier/resolver retarget contract by `test_classification`.
