...
Wiring happens in `cmd/spex/main.go`, which builds the root and attaches all thirteen constructors in a single registration call, in this order: `hash-id`, `diff`, `validate`, `impact`, `map`, `register`, `log`, `template`, `version`, `render`, `emit`, `ingest`, `upgrade`.
