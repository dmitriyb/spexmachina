...
The graphs are the reference-kind fields the resolved profile declares, minus those marked `cyclic: true` — that optional flag on a reference field declaration marks a descriptive edge exempt from the cycle check, and an omitted flag means cycle-checked — plus one edge of the frame's own: `requires_module`, which is not the profile's to declare and is always cycle-checked, never exemptable. One graph is built per non-exempt reference kind and source occurrence; beyond the frame's fixed module edge, the checker holds no edge list of its own. The default profile omits the flag on all six of its reference kinds, so every one is checked — four of the six vacuously: `implements`, `provided_by` and `describes` because a kind whose source and target types differ cannot close a loop, and `preq_id` because its source and target are both requirements but the target scope's requirements carry no such field, so no loop can close there either — while `depends_on`, `uses` and the frame's `requires_module` can. `depends_on` is walked at both scopes its `requirement` source is declared at, so the default yields four graphs that can actually cycle:
