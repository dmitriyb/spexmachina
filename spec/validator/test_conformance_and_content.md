...
**When** `CheckContentPaths(specDir)` is called.
**Then** one `content` error for the endpoint's declared path — ContentResolver walks whichever types the resolved profile marks content-bearing, not a fixed three; under the default profile the walked set is exactly components, data_flows and test_sections, and all existing scenarios above hold unchanged.
