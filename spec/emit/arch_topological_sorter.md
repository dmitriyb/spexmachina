...
[[f379c06c6a53|Order is decided on two levels at once]], the outer one by node kind and the inner one by the spec graph:
...
Given the batch's create actions, the sorter answers with those same actions in emitted order, plus a provisional op_id per action and a spec_node_id-to-op_id map built from it. ChangesetBuilder keeps the order and discards both: it renumbers every op itself once the close ops are counted, and rebuilds the map from its own numbering.
...
- Ops are numbered from 1 in emitted order, so reading the changeset top to bottom reads the creates in sort order. The id strings and their zero-padding width are ChangesetBuilder's: the width covers the close ops too, and the sorter is handed only the creates.
