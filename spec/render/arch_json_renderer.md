...
Every node carries `id`, `type` and `name`; `description`, `content`, `module` and `group` appear only where the declaration has them, so an api carries no `content` key at all and a test section carries no `description`. The types emitted are the resolved profile's declared types — a profile-declared type's nodes appear with their declared type name in `type`, content inlined where the type is content-bearing — and under the default profile the emitted set is exactly today's. The document is indented two spaces, and both arrays follow one declaration walk — the project node, the project's requirements and sections, then each module in turn contributing its own node and its declared arrays in order: under the default profile, its requirements, its components, its data flows, its test sections and its apis. Declaration order is the only order there is, which is what makes two renderings of one unchanged spec byte-identical.
