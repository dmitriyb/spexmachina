...
The one graph the command wires through is read through the resolved profile, so every declared node type reaches whichever formats project its kind — `spex render` is where a resolved profile is observable without a dedicated command, the slim JSON node table included. The command itself branches on no type name; the profile's reach ends inside SpecReader and the three renderers.
