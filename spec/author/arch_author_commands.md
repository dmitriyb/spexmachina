...
| [[1aaff8ac0ae3|`spex node set`]] | NodeEditor |
...
`spex node`, `spex edge`, `spex leaf` and `spex profile` are groupings: a bare one prints its grouping's help and exits 0, runs nothing and carries no contract, so none is declared as an api and the nine children are. That is the opposite of `spex map`, whose parent is a declared api; the difference is deliberate and local — a grouping here owns nothing a child does not, so a node for it would carry no identity a child does not already carry as its first two words. Five constructors reach the root's registration list — `node`, `edge`, `leaf`, `profile`, `migrate` — and nine api nodes are declared here, in the module that owns the entry points, which is what lets `provided_by` stay module-local. A child added under an existing grouping, as `spex node set` was, adds no constructor: the root's bounded count does not move and the cli module owes nothing for it.
...
Five flags are fixed by the spec: the root's persistent `--spec-dir`, honoured by every surface; `--type` on `spex node add`, taking a type the resolved profile declares or the frame value `module`; `--force` on `spex node remove`, which performs a removal over inbound references and lists them as dangling; and `--field name=value`, repeatable, and `--unset name` on `spex node set`, which may appear together in one invocation, with a name given to both refused. The rest of the vocabulary — how the name, the module, the declared field values, an edge's source, field and target, and a rename's new name are passed — is not decided here or in the proposal; it is the implementer's choice, and the authoring loop records it in each api's `description` once chosen, since that is the one field a surface is written down in. `spex node set` arrived after that vocabulary was settled, which is why its two flags are fixed above rather than left to the implementer: `--field` is the form add settled on, and `--unset` has no precedent to follow.
...
- `2` — contract refusal: [[b9e7b96f6aa7|ObligationReporter]] refused the change, or a worker refused a field, node or leaf another surface owns. The error document, each entry with its `fix`, is on stdout; an ownership refusal carries the owning surface as its `fix` and takes this code, not `1`, because the input was well-formed and the tree was left untouched.
