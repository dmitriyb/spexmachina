...
It does not read `.spex/`: the before-state is the tree on disk, not the baseline snapshot, so the obligations of one command are relative to the previous command's result — what this write leaves open, not a running total: a later command can close an earlier one's obligation, and `spex diff` against the session's starting snapshot is the one computation that reports the session as a whole. It does not write: the caller writes, after the reporter has answered, or does not. And it carries no profile knowledge of its own beyond what the checkers and the completeness rules already read from the resolved profile handed to it.
